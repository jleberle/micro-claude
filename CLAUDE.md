...
### `/feed.xml` is theme-blank's template — we deliberately do NOT own it

Verified by fingerprint match against the live feed (`xmlns:source`,
`<source:markdown>`, `first 25`, empty `<description/>`): the RSS feed is
rendered by **theme-blank's `layouts/index.xml`**, not by anything here. We could
shadow it — same mechanism as the nine paths we do win — and there are two
temptations to: it uses the deprecated `.Site.LanguageCode`, and it has real RSS
2.0 defects (empty required channel `<description>`, 25 empty `<title/>`
elements, and every `<guid>` omitting `isPermaLink="false"` while differing from
its own `<link>`).

Do not. Two reasons, in order:

1. micro.blog **cannot** ship a Hugo that removes `.Site.LanguageCode` without
   breaking every site's feed on the platform, so fixing theme-blank is a
   precondition of their upgrade, not a follow-up. The insurance has near-zero
   expected value.
2. Owning it means permanently diverging: whatever micro.blog later adds to that
   template — namespaces, podcast tags, feed-spec fixes — we silently stop
   receiving. That cost is unbounded and recurring; the defects above are not.

Reimplementation risk is real but secondary: `.Params.guid` is
`http://jleberle.micro.blog/<path>`, differing from `<link>` on 100% of items
(all 187, 2012–2026 — it is the account's stable internal identity, not a legacy
remnant). A rewrite that fell through to `.Permalink` would re-show every item as
new in every subscriber's reader.
...
- `og:locale` is hardcoded `en_US` because that is the format Open Graph wants
  (language_TERRITORY) and no Hugo variable supplies it. An earlier note here
  claimed `.Site.LanguageCode` "returns the literal `-`" — **that is false**.
  Tested on 0.158: it returns `en` unset, `en-us` with `locale` set, `en` with
  `languageCode` set, never `-`. Production agrees — the live feed's
  `<language>en</language>` is generated by that exact expression. The hardcoding
  is still correct; only the stated reason was wrong.
